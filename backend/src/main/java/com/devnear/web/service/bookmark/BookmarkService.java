...
import com.devnear.web.domain.bookmark.BookmarkPortfolio;
import com.devnear.web.domain.bookmark.BookmarkPortfolioRepository;
...
public class BookmarkService {
    ...
    @Transactional
    public void addPortfolioBookmark(User user, Long portfolioId) {
        ClientProfile clientProfile = findClientProfileByUser(user);
    ...
    @Transactional
    public void removePortfolioBookmark(User user, Long portfolioId) {
        ClientProfile clientProfile = findClientProfileByUser(user);
        Portfolio portfolio = portfolioRepository.findById(portfolioId)
                .orElseThrow(() -> new ResourceNotFoundException("포트폴리오를 찾을 수 없습니다."));

        BookmarkPortfolio bookmark = bookmarkPortfolioRepository
                .findByClientProfileAndPortfolio(clientProfile, portfolio)
                .orElseThrow(() -> new ResourceNotFoundException("찜한 포트폴리오가 아닙니다."));

        bookmarkPortfolioRepository.delete(bookmark);
    }

    public Page<PortfolioResponse> getBookmarkedPortfolios(User user, Pageable pageable) {
        ClientProfile clientProfile = findClientProfileByUser(user);
        return bookmarkPortfolioRepository.findAllByClientProfile(clientProfile, pageable)
                .map(bookmark -> PortfolioResponse.from(bookmark.getPortfolio()));
    }
